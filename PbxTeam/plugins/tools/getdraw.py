import httpx
import os
from PbxTeam.plugins.tools.getdraw import DEEP_API
...
DEEP_API = "f235d628-c1ca-48b1-bbeb-77efd8d4671c"
...
@app.on_message(cdz(["getdraw"])  & (filters.me | filters.user(SUDO_USER))
)
async def draw_image(client, message):
    ...
    if not PbxTeam.plugins.tools.getdraw.DEEP_API:
    ...
    data = requests.post(
        ...
        headers={'api-key': PbxTeam.plugins.tools.getdraw.DEEP_API}
    ).json()
